...
\begin{itemize}
    \item Presentation, description and documentation of our results through scientific publications in peer-reviewed journals. We will make use of open access journals and we will always make our publications available through \url{https://arxiv.org}. The latter repository is listed in the Registry of Open Access Repositories as \url{http://roar.eprints.org/89/}, and is the most popular repository for the fields of astronomy and astrophysics. We will deposit all our journal articles in this repository either upon submission or upon acceptance of the articles in question (the choice is left up to the authors). In this way the article is openly accessible (as PDF and in original source format, typically \LaTeX) in parallel to any period during which the journal article is only open to subscribers (this is typically a one-year period for astronomical journals). Hence, although the journal articles are `green open access', depositing them with \texttt{arXiv.org} in parallel effectively ensures `gold open access'.
